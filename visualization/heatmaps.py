"""Heatmap visualizations — warm palette."""
...
def bias_heatmap(profile_df: pd.DataFrame) -> go.Figure:
    """Heatmap: rows=models, columns=D1..D4, cell color = mean score."""
    models = profile_df["Model"].tolist()
    dims = ["D1", "D2", "D3", "D4"]
    value_cols = ["D1_mean", "D2_mean", "D3_mean", "D4_mean"]
    ...
    fig.update_layout(
        **LAYOUT_BASE,
        title=dict(text="Bias Score Heatmap (Model × Dimension)", font=CHART_TITLE_FONT),
...
def model_comparison_heatmap(analyses_df: pd.DataFrame, dimension: str = "tr_score") -> go.Figure:
    ...
    dim_labels = {
        "tr_score": "TR Score",
        "d1_score": "D1: Responsibility Attribution",
        "d2_score": "D2: Coverage Balance",
        "d3_score": "D3: Rule Citation",
        "d4_score": "D4: Final Framing",
    }
